const acceptReq = async (body, redis, callback) => {
    ...
    pipeline.call("GRAPH.QUERY", "Employee", `MATCH(e:Employee) WHERE ID(e)=${body['id']} MATCH(req:Req) WHERE ID(req)=${body['reqId']} MATCH(req)--(newTeam:Team)  MATCH (e)-[rel:Is_Part_Of]-(:Team) DELETE rel DELETE req MERGE(e)-[r:Is_Part_Of]->(newTeam)`)
    expectedResponses += 1
